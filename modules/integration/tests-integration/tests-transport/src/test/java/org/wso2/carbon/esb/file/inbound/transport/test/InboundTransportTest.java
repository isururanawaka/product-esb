...
public class InboundTransportTest extends ESBIntegrationTest {
	...
	public void testInboundEnpointReadFile_ContentType_XML() throws Exception {
		...
		File sourceFile = new File(pathToFtpDir + File.separator + "test1.xml");
		...
		for (LogEvent logEvent : logs) {
			...
			if (message.contains("<m0:symbol>WSO2-Colombo</m0:symbol>")) {
				isFileRead = true;
			}
		}
	...
	@SetEnvironment(executionEnvironments = { ExecutionEnvironment.STANDALONE })
	@Test(groups = { "wso2.esb" }, description = "Inbound Endpoint invalid interval Test case")
	...
	@SetEnvironment(executionEnvironments = { ExecutionEnvironment.STANDALONE })
	@Test(groups = { "wso2.esb" }, description = "Inbound Endpoint invalid File URI Test case")
	...
	@SetEnvironment(executionEnvironments = { ExecutionEnvironment.STANDALONE })
	@Test(groups = { "wso2.esb" }, description = "Inbound Endpoint File name with special chars URI Test case")
	...
	@SetEnvironment(executionEnvironments = { ExecutionEnvironment.STANDALONE })
	@Test(groups = { "wso2.esb" }, description = "Inbound Endpoint Content type invalid Test case")
	...
	@SetEnvironment(executionEnvironments = { ExecutionEnvironment.STANDALONE })
	@Test(groups = { "wso2.esb" }, description = "Inbound Endpoint Content type not specified Test case")
	...
	@SetEnvironment(executionEnvironments = { ExecutionEnvironment.STANDALONE })
	@Test(groups = { "wso2.esb" }, description = "Inbound Endpoint move after process Test case")
	public void testInboundEndpointMoveAfterProcess() throws Exception {

		addInboundEndpoint(addEndpoint8());

		File sourceFile = new File(pathToFtpDir + File.separator + "test.xml");
		File targetFolder = new File(InboundFileFolder + File.separator + "move");
		File targetFile = new File(targetFolder + File.separator
		                           + "test.xml");
		File processedFolder = new File(InboundFileFolder + File.separator + "processed");
	...
	//
	//		addInboundEndpoint(addEndpoint9());
	//
	//		File sourceFile = new File(pathToFtpDir + File.separator + "test1.xml");
	//		File targetFolder = new File(InboundFileFolder + File.separator + "in");
	//		File targetFile = new File(targetFolder + File.separator
	...
	//		}
	//
	//		File processedFile = new File(processedFolder + File.separator + "test1.xml");
	//		try {
	//			FileUtils.copyFile(sourceFile, targetFile);
	//			Thread.sleep(2000);
	//
	...
	//	private OMElement addEndpoint9() throws Exception {
	//		OMElement synapseConfig = null;
	//		synapseConfig = AXIOMUtil
	//				.stringToOM("<inboundEndpoint name=\"testFile9\" onError=\"inFault\" protocol=\"file\"\n"
	//						+ " sequence=\"requestHandlerSeq2\" suspend=\"false\" xmlns=\"http://ws.apache.org/ns/synapse\">\"\n"
	//						+ " <parameters>\n"
	//						+ " <parameter name=\"interval\">1000</parameter>\n"
	//						+ " <parameter name=\"transport.vfs.ActionAfterErrors\">NONE</parameter>\n"
	//						+ " <parameter name=\"transport.vfs.Locking\">enable</parameter>\n"
